...
class MPCController : public Controller {
 ...
 protected:
  ...
  // a proxy to analyze the planning trajectory 车道轨迹线的代理分析器
  TrajectoryAnalyzer trajectory_analyzer_;

  void LoadControlCalibrationTable(
      const MPCControllerConf &mpc_controller_conf); 

  void LoadMPCGainScheduler(const MPCControllerConf &mpc_controller_conf); // 加载MPC模型的迭代增益函数
  ...
  double ts_ = 0.0;
  // corner stiffness; front 前轮胎侧偏刚度
  double cf_ = 0.0;
  // corner stiffness; rear  后轮胎****
  double cr_ = 0.0;
  // distance between front and rear wheel center 前后轮心距
  double wheelbase_ = 0.0;
  // mass of the vehicle 
  double mass_ = 0.0;
  // distance from front wheel center to COM 前轮心到质心的距离
  double lf_ = 0.0;
  // distance from rear wheel center to COM 
  double lr_ = 0.0;
  // rotational inertia 转动惯量/惯性矩
  double iz_ = 0.0;
  // the ratio between the turn of the steering wheel and the turn of the wheels 前后轮转动比
  double steer_ratio_ = 0.0;
  // the maximum turn of steer // 最大转向角
  double steer_single_direction_max_degree_ = 0.0;
  // the maximum turn of vehicle wheel // 车轮最大转向角
  double wheel_single_direction_max_degree_ = 0.0;

  // limit steering to maximum theoretical lateral acceleration 最大加速下转向限制角度
  double max_lat_acc_ = 0.0;

  // number of states, includes 各状态的误差系数
  // lateral error, lateral error rate, heading error, heading error rate,
  // station error, velocity error,
  const int basic_state_size_ = 6; // 6个状态系数
  ...
  // updated state weighting matrix 更新后的状态权重矩阵
  Eigen::MatrixXd matrix_q_updated_;
  // vehicle state matrix coefficients 状态矩阵系数？？？
  Eigen::MatrixXd matrix_a_coeff_;
  // 4 by 1 matrix; state matrix 这是个什么东西？？
  Eigen::MatrixXd matrix_state_;

  // heading error of last control cycle 控制点到参考轨迹点的纵向误差
  double previous_heading_error_ = 0.0;
  // lateral distance to reference trajectory of last control cycle ******横向误差
  double previous_lateral_error_ = 0.0;

  // parameters for mpc solver; number of iterations mpc迭代次数
  int mpc_max_iteration_ = 0;
  // parameters for mpc solver; threshold for computation 计算阈值？？
  double mpc_eps_ = 0.0;
